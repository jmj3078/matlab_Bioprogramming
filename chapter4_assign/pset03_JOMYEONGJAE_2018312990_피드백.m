...
%내장 함수 sort를 사용하는 것이 for문을 활용하는 것 보다 더 빨랐습니다.

% prof comment: 탐구 정신 good. 
% prof comment: MATLAB의 for가 느리기 때문임. 내장 함수들은 MATLAB보다 빠른 언어로 만들어졌음. 
% prof comment: 계산 횟수의 측면에서는 (for 사용) ~ (내장 min) << (내장 sort)
% prof comment: min 함수를 만들기 위해 min 보다 더 복잡한 sort를 사용하는 것은 낭비. 
...
% prof comment: 결과 비교할 때 동일성 확인도 코드로 
% prof comment: 30 - 5 + 보너스 5 = 30
...
% prof comment: all good. 10 + 10 + 10 + 15 = 45
...
TF = isvector(vec);
    if TF == 0
        ...
        min = vec(1);
        % prof comment: 첫번째 자리에 최솟값 있을 경우 에러이므로 추가 필요 (-5)
        % index = 1; 
        for ii = (2:numel(vec))
            if min > vec(ii)
                min = vec(ii);
...
function M = matrix_multiply_1(A, B)
    ...
    % prof comment: 한 줄로 가능. [rown_A, coln_A] = size(A); 
