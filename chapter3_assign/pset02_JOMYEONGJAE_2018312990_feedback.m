...
% prof comment: 정확하고 간편한 비교를 위해 두 결과를 코드로 비교하기
% prof comment: 30점 
...
% prof comment: 테스트 결과값 주의 깊게 확인 바람 (-15) 
% prof comment: 40 - 15 = 25점 
...
function [even, odd] = even_odd(rand_int)
    ...
    log_odd = ~log_even;        % prof comment: good
...
% prof comment: 열심히 해서 좋습니다. 다만, 문제의 의도는 단순한 것이었음. 
% prof comment: 동점자가 있을 때 보통 1,1,3등 으로 표현하므로 중복 데이터를 제거하는 것은 통상적 해석과 달라질 수 있음.
% prof comment: (1,1,3등은 곧 2,2,3등과도 같음)
...
function shifted_m = shift_R(matrix, n)
    ...
    shifted_col_index = mod([1:col_num] + n, col_num) + 1; % prof comment: 답안 코드. 열번호 계산 오류.
    shifted_m = matrix(:, shifted_col_index);              % prof comment: 답안 코드. 열번호 계산 오류. 
    
    shifted_col_index = mod( (1:col_num) + n - 1, col_num) + 1; % prof comment: 채점 중 수정 코드
    shifted_m(:, shifted_col_index) = matrix; 
    
end

%%
% prof comment: 20 + 30 + 30 + 25 + 열심 보너스 5 = 110점 
