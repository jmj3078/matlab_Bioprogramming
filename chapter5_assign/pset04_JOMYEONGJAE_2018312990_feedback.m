...
% prof comment: 계산 결과를 여러번 사용할 일이 있으면 변수에 저장.
% prof comment: 변수로 disp하고 변수로 movmean 결과와 비교. 
...
%모두 똑같음
% prof comment: 1000개 배열의 비교 결과는 isequal, all등을 이용해서 요약 출력하는 게 좋음 
...
for ii = 1:nrow_species
    ...
    % prof comment: find가 없어도 idx_grid로 쓸 수 있음 
...
% prof comment: 기존 추측값과 같은 값을 다시 제시하는 경우가 생김. (-5)
% guess_num = randi([guess_min+1, guess_max-1])
...
% prof comment: 특수한 경우가 아니면 굳이 nan을 쓸 필요는 없고 보통은 zeros를 쓰는 것이 좋음 
% prof comment: 이 풀이에서는 movmean에 계산한 결과를 덮어쓰므로 nan과 0 어느 쪽이든 차이 없음. 
